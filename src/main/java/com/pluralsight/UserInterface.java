...
public class UserInterface {
    ...
    private void orderRamen() {
        ...
        if (orderList.get(orderList.size() - 1) instanceof Ramen) {
            switch (userInput) {
                case "1":
                    ((Ramen) orderList.get(0)).addRamenSize(1);
                    System.out.println("You selected Small for your bowl size");
                    break;
                case "2":
                    ((Ramen) orderList.get(0)).addRamenSize(2);
                    System.out.println("You selected Medium for your bowl size");
                    break;
                case "3":
                    ((Ramen) orderList.get(0)).addRamenSize(3);
        ...
        if (orderList.get(orderList.size() - 1) instanceof Ramen) {
            switch (userInput) {
                case "1":
                     ((Ramen) orderList.get(orderList.size() - 1)).addToppings(1);
                    System.out.println("Extra noodles added");
                     break;
                case "2":
                    ((Ramen) orderList.get(orderList.size() - 1)).addToppings(2);
                    System.out.println("Extra Chashu 2 pieces added");
                    break;
                case "3":
                    ((Ramen) orderList.get(orderList.size() - 1)).addToppings(3);
                    System.out.println("Extra soft boiled egg added");
                    break;
                case "4":
                    ((Ramen) orderList.get(orderList.size() - 1)).addToppings(4);
                    System.out.println("Extra whole fried garlic added");
                    break;
                case "5":
                    ((Ramen) orderList.get(orderList.size() - 1)).addToppings(5);
                    System.out.println("Extra bamboo shoots added");
                    break;
            }
        }

        order.addMenuItem(orderList.get(orderList.size() - 1));
